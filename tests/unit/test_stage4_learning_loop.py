...
class TestStage4LearningLoop(unittest.TestCase):
    def test_dynamic_goal_is_understood_without_being_fabricated(self):
        ...
        self.assertEqual(spec.source, "dynamic")
        self.assertFalse(spec.resource_available)
        self.assertGreater(len(spec.competencies), 2)

    def test_mastery_and_history_are_persistent(self):
        d = Path(tempfile.mkdtemp())
        try:
            store = ProfileStore(str(d / "p.json"))
            store.update("u", goal_id="goal_ai_engineer")
            store.set_mastery("u", "docker_basics", 0.45, "assessment")
            self.assertEqual(store.get("u")["mastery"]["docker_basics"], 0.45)
            self.assertIn("docker_basics", store.get("u")["unmastered_skill_ids"])
            store.set_mastery("u", "docker_basics", 0.92, "assessment")
    ...
    def test_real_checkpoint_scoring(self):
        ...
        self.assertEqual(
            score_answers(
                "docker_basics",
                {"docker_basics_1": 0, "docker_basics_2": 1, "docker_basics_3": 1},
            ),
            66.7,
        )
    ...
    def test_projects_are_first_class(self):
        ...
        self.assertEqual(p["project_id"], "project_rag_systems")
        self.assertIn("rag_systems", p["skills"])
