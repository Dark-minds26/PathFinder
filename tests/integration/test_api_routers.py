...
class TestAPIRouters(unittest.TestCase):
    ...
    def test_health_check(self):
        ...
        self.assertIn(body["explainer_backend"], {"ConfiguredExplainer", "ShapTreeExplainer"})
        self.assertIn(body["llm_backend"], {"GroqClient", "OpenAIClient", "LocalStubLLMClient"})
    ...
    def test_full_flow_new_user_chat_to_path_to_explain(self):
        ...
        self.client.post(
            "/profile/chat",
            json={"user_id": user_id, "message": "I know python basics, beginner level, visual learner, 8 hours per week"},
        )
    ...
    def test_chat_profile_change_returns_updated_path(self):
        ...
        data = json.loads(store.path.read_text()) if store.path.exists() and store.path.read_text().strip() else {}
        data.pop(user_id, None); store.path.write_text(json.dumps(data, indent=2, sort_keys=True))
        r1 = self.client.post("/profile/chat", json={"user_id": user_id, "message": "I want to become an AI engineer"})
        self.assertEqual(r1.status_code, 200)
        self.assertTrue(r1.json()["roadmap_updated"])
        r2 = self.client.post("/profile/chat", json={"user_id": user_id, "message": "I learn through practical projects and can study 7 hours a day"})
    ...
    def test_profile_hydration_endpoint(self):
        ...
        data.pop(user_id, None); store.path.write_text(json.dumps(data, indent=2, sort_keys=True))
        try:
            self.client.post("/profile/chat", json={"user_id": user_id, "message": "I want to become an AI engineer"})
...
def _cleanup_live_profile(user_id):
    from api.dependencies import get_profile_store
    import json
    store=get_profile_store(); path=store.path
    data=json.loads(path.read_text()) if path.exists() and path.read_text().strip() else {}
    data.pop(user_id,None); path.write_text(json.dumps(data,indent=2,sort_keys=True))
...
class TestStage3APIContracts(unittest.TestCase):
    ...
    def test_generate_requires_goal(self):
        uid='stage3_no_goal_user'; _cleanup_live_profile(uid)
        r=self.client.post('/path/generate',json={'user_id':uid})
        self.assertEqual(r.status_code,400); self.assertEqual(r.json()['detail'],"Tell me what role you're targeting before generating a path.")

    def test_assessment_score_is_validated(self):
        r=self.client.post('/assessment/submit',json={'user_id':'u','skill_id':'python_basics','score':101})
        self.assertEqual(r.status_code,422)

    def test_mastered_failed_reintroduced_transition(self):
        uid='stage3_reroute_user'; _cleanup_live_profile(uid)
        try:
            self.client.post('/profile/chat',json={'user_id':uid,'message':'I want to become a backend engineer'})
            self.client.post('/profile/chat',json={'user_id':uid,'message':'I already know Python basics'})
            ...
            before=self.client.post('/path/generate',json={'user_id':uid}); self.assertEqual(before.status_code,200)
            self.assertNotIn('python_basics',{x['skill_id'] for x in before.json()['path']})
            failed=self.client.post('/assessment/submit',json={'user_id':uid,'skill_id':'python_basics','score':40}); self.assertEqual(failed.status_code,200)
            after=failed.json(); self.assertIn('python_basics',{x['skill_id'] for x in after['path']})
            from api.dependencies import get_profile_store
            self.assertNotIn('python_basics',get_profile_store().get(uid)['skill_ids'])
        finally: _cleanup_live_profile(uid)

    def test_frontend_contracts(self):
        html=Path('api/static/index.html').read_text()
        self.assertIn('id="generate-btn"',html)
        self.assertNotIn("score.textContent = 'predicted score ' + step.predicted_score",html)
        self.assertIn("Tell me what role you're targeting before generating a path.",html)
...
class TestPhase3AssessmentLifecycle(unittest.TestCase):
    ...
    def test_api_mastered_failed_reintroduced_and_revalidated(self):
        ...
        try:
            ...
            failed = self.client.post('/assessment/submit', json={
                'user_id': uid, 'skill_id': 'python_basics',
                'answers': {'python_basics_1': 0, 'python_basics_2': 0, 'python_basics_3': 0}
            })
            self.assertEqual(failed.status_code, 200)
            p = store.get(uid)
            self.assertEqual(p['mastery_state']['python_basics']['status'], 'failed')
            self.assertIn('python_basics', {x['skill_id'] for x in failed.json()['path']})
            self.assertTrue(any(x.get('type') == 'assessment_failed' and x.get('previous_status') == 'validated' for x in p['learning_history']))

            # Passing a real checkpoint validates it and removes it from the path again.
            passed = self.client.post('/assessment/submit', json={
                'user_id': uid, 'skill_id': 'python_basics',
                'answers': {'python_basics_1': 1, 'python_basics_2': 1, 'python_basics_3': 1}
            })
            self.assertEqual(passed.status_code, 200)
            p = store.get(uid)
            self.assertEqual(p['mastery_state']['python_basics']['status'], 'validated')
            self.assertTrue(any(x.get('type') == 'assessment_validated' and x.get('previous_status') == 'failed' for x in p['learning_history']))
            self.assertNotIn('python_basics', {x['skill_id'] for x in passed.json()['path']})
        finally:
            _cleanup_live_profile(uid)
