...
def compute_attributions(
    ...
    try:
        missing = set(
            ctx.missing_skills_for_user(
                user_id, goal_id=goal_id, possessed_skills=possessed_skills
            )
        )
        ...
        top_k = min(
            len(ranked), max(1, int(getattr(explainer, "top_k_features", len(ranked))))
        )
        ...
        if len(selected) < top_k:
            ...
            selected.extend(
                item
                for item in ranked
                if item[0] not in required and item[0] not in selected_names
            )
