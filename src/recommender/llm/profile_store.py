...
class ProfileStore:
    ...
    def _ensure_mastery_state(profile: dict) -> None:
        ...
        has_legacy_mastery = bool(
            profile.get("mastery")
            or profile.get("skill_ids")
            or profile.get("unmastered_skill_ids")
            or profile.get("mastery_state")
        )
        ...
        for sid, raw_score in mastery.items():
            ...
            status = (
                "validated"
                if score >= 0.80
                else ("needs_review" if score >= 0.60 else "failed")
            )
            states[sid] = Mastery(
                skill_id=sid,
                score=score,
                status=status,
                source=sources.get(sid, "legacy"),
            ).model_dump(mode="json")
        for sid in profile.get("skill_ids", []):
            if sid not in states:
                states[sid] = Mastery(
                    skill_id=sid, score=0.80, status="validated", source="self_report"
                ).model_dump(mode="json")
        ...
        for sid in profile.get("unmastered_skill_ids", []):
            if sid not in states:
                states[sid] = Mastery(
                    skill_id=sid, score=0.0, status="needs_review", source="legacy"
                ).model_dump(mode="json")
    ...
    def update(
        ...
        *,
        ...
        weekly_hours=None,
        new_skill_ids=None,
        unmastered_skill_ids=None,
        interests=None,
        roadmap_preferences=None,
        goal_spec=None,
    ) -> dict:
        try:
            with self._lock:
                ...
                if weekly_hours is not None:
                    ...
                    if hours <= 0 or hours > 168:
                        raise ValueError(
                            "weekly_hours must be greater than 0 and no more than 168"
                        )
                    profile["weekly_hours"] = hours
                if interests:
                    profile["interests"] = sorted(
                        set(profile.get("interests", [])) | set(interests)
                    )
                if roadmap_preferences:
                    profile["roadmap_preferences"] = {
                        **profile.get("roadmap_preferences", {}),
                        **{k: bool(v) for k, v in roadmap_preferences.items() if v},
                    }
                if new_skill_ids:
                    ids = set(new_skill_ids)
                    profile["skill_ids"] = sorted(
                        set(profile.get("skill_ids", [])) | ids
                    )
                    mastery = profile.setdefault("mastery", {})
                    sources = profile.setdefault("mastery_source", {})
                    for sid in ids:
                ...
                if unmastered_skill_ids:
                    profile["skill_ids"] = sorted(
                        set(profile.get("skill_ids", [])) - set(unmastered_skill_ids)
                    )
                    profile["unmastered_skill_ids"] = sorted(
                        set(profile.get("unmastered_skill_ids", []))
                        | set(unmastered_skill_ids)
                    )
                self._ensure_mastery_state(profile)
    ...
    def complete_course(self, user_id, course_id, skill_id=None):
        with self._lock:
            data = self._read_all()
            p = data.setdefault(user_id, {"skill_ids": []})
            p["completed_course_ids"] = sorted(
                set(p.get("completed_course_ids", [])) | {course_id}
            )
            p.setdefault("learning_history", []).append(
                {
    ...
    def set_mastery(
        self, user_id: str, skill_id: str, mastery: float, source: str = "assessment"
    ) -> dict:
        try:
            with self._lock:
                data = self._read_all()
                ...
                profile.setdefault("mastery_state", {})[skill_id] = Mastery(
                    skill_id=skill_id, score=score, status=status, source=source
                ).model_dump(mode="json")
                profile.setdefault("mastery_source", {})[skill_id] = source
                if status == "validated":
                    profile["skill_ids"] = sorted(
                        set(profile.get("skill_ids", [])) | {skill_id}
                    )
                    profile["unmastered_skill_ids"] = [
                        x
                        for x in profile.get("unmastered_skill_ids", [])
                        if x != skill_id
                    ]
                else:
                    ...
                    profile["skill_ids"] = [
                        x for x in profile.get("skill_ids", []) if x != skill_id
                    ]
                    profile["unmastered_skill_ids"] = sorted(
                        set(profile.get("unmastered_skill_ids", [])) | {skill_id}
                    )
                data[user_id] = profile
                self._write_all(data)
                return profile
    ...
    def record_history(self, user_id: str, item: dict) -> dict:
        try:
            with self._lock:
                data = self._read_all()
                p = data.setdefault(user_id, {"skill_ids": []})
                p.setdefault("learning_history", []).append(item)
                data[user_id] = p
                self._write_all(data)
                return p
        except Exception as e:
    ...
    def completeness(self, user_id: str) -> float:
        ...
        checks = [
            bool(p.get("goal_id")),
            bool(p.get("experience_level")),
            bool(p.get("learning_style")),
            p.get("weekly_hours") is not None,
            bool(p.get("skill_ids") or p.get("unmastered_skill_ids")),
        ]
    ...
    def mark_unmastered(self, user_id, skill_id):
        try:
            with self._lock:
                data = self._read_all()
                p = data.get(user_id, {"skill_ids": []})
                p["skill_ids"] = [s for s in p.get("skill_ids", []) if s != skill_id]
                p["unmastered_skill_ids"] = sorted(
                    set(p.get("unmastered_skill_ids", [])) | {skill_id}
                )
                data[user_id] = p
                self._write_all(data)
                return p
